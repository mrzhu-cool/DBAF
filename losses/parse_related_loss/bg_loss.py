...
import numpy as np
from losses.parse_related_loss.unet import unet
...
class ParseLoss(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.parsenet.eval()
        #self.bg_mask_l2_loss = torch.nn.MSELoss(size_average='mean')
        self.shrink = torch.nn.AdaptiveAvgPool2d((512, 512))#收缩
        #self.magnify = torch.nn.AdaptiveAvgPool2d((1024, 1024))
        self.magnify = torch.nn.AdaptiveAvgPool2d((opts.stylegan_size, opts.stylegan_size))#放大
        self.select_list=[2,4,5,10,11,12]

    def predict(self,x):
        # x_predict = self.magnify(self.parsenet(self.shrink(x)))
        x_predict = self.parsenet(x)
    ...
    def gen_mask(self,input_image):
        ...
        labels_predict = self.parsenet(input_image).detach()
        #mask_512 = (torch.unsqueeze(torch.max(labels_predict, 1)[1], 1)!=13).float()
        mask_256 = (torch.unsqueeze(torch.max(labels_predict, 1)[1], 1)!=0).float() #0 for background
        #mask_1024 = self.magnify(mask_512)
        return mask_256
