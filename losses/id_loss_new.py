...
class IDLossExtractor(torch.nn.Module):
    def __init__(self,opts,requires_grad=False):
        #requirement input 256*256 cropped and normalized face image tensor
        self.opts=opts
        super(IDLossExtractor, self).__init__()
        self.facenet = Backbone(input_size=112, num_layers=50, drop_ratio=0.6, mode='ir_se').to(opts.device)
        self.facenet.load_state_dict(torch.load(
            opts.id_encoder_path, map_location=opts.device))
        self.pool = torch.nn.AdaptiveAvgPool2d((256, 256))
        self.face_pool = torch.nn.AdaptiveAvgPool2d((112, 112))
        ...
        self.output_layer=self.facenet.output_layer
        self.input_layer=self.facenet.input_layer
        body = self.facenet.body
    ...
    def calculate_loss(self,features1,features2):
        ...
        rev_id_loss=torch.cosine_similarity(features1[-1],features2[-1]).clamp(min=self.margin).mean()#clamp控制最大最小值
    ...
    def forward(self,img1,img2):
        img1 = img1.to(self.opts.device)
        img2 = img2.to(self.opts.device)
    ...
    def calculate_contrastive_loss(self,img1,img2):
        ...
        id_loss=((1-torch.cosine_similarity(features1[-1],features2[-1])).clamp(min=self.margin))
